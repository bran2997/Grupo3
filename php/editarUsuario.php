<?php
	if(isset($_POST['boton']))
	{
		...
		if($contrasenna == $contrasenna2)
		{
			$objeto = new dataBase();
			$obje = $objeto;
			$result = $objeto->consultar("SELECT * FROM usuario where idUsuario='".$id."';");
			$todo = "";
			$objeto = 0;
			$flag = True;
			while (($fila = mysqli_fetch_array($result)) and $flag )
			{
				...
				{
					if(($fila['direccionUsuario'] == $usuarioViejo) && ($fila['idUsuario'] == $id))
					{
						$flag = true;
					}
					else
			...
			if ($flag)
			{
				if($nombre!="" and $apellido!="" and $usuario!="" and $contrasenna!="" and $nacimiento!="")
				{
					...
					$consulta = "update usuario  set 
						...
						if ($resultado)
						{
							$consulta = "select * from usuario here idUsuario='".$id."';";
